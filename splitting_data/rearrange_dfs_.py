...
def rearrange_df(
    files, start, end, split, size, data_path, output_path, identifier, delete=False
):
    former = []
    already_written = []
    big_df = pd.DataFrame()
    ...
    while len(files) > 0:
        ...
        if free < 2**30 * 24:
            raise NotEnoughDiskSpaceError(
                f"Free disk space is less than 24 GiB. Free: {free // (2**30)} GiB"
            )
        ...
        if len(big_df.index) < size:
            ...
            former.append(
                (file, len(from_file.index), i)
            )  # Since I want to keep track of the files I have processed because we might need to delete them later
        ...
        if delete:
            ...
            files_passed = i - oldest_file[2]
            if oldest_file[1] <= (
                size * (files_passed)
            ):  # only if the file could have been written
                ...
                old_df = pd.read_pickle(oldest_file[0])
                oldest_file_subset = set(old_df["lexical_word"].unique())
                ...
                if oldest_file_subset.issubset(check_df_subset):
                    ...
                    logging.info(
                        f"Cannot delete {oldest_file[0]} because not all words have been written"
                    )
        else:
...
if __name__ == "main":
    ...
    parser.add_argument(
        "--language", type=str, help="Language of the data.", default="en"
    )
    ...
    parser.add_argumennt(
        "--ingore", type=str, help="Ignore files with this Regex.", default=""
    )
    parser.add_argument(
        "--identifier",
        type=str,
    ...
    parser.add_argument(
        "--start_test", type=int, help="Start index for test set.", default=0
    )
    parser.add_argument(
        "--end_test", type=int, help="End index for test set.", default=-1
    )
    parser.add_argument(
        "--start_validation",
        type=int,
        help="Start index for validation set.",
        default=0,
    )
    parser.add_argument(
        "--end_validation", type=int, help="End index for validation set.", default=-1
    )
    parser.add_argument(
        "--start_training", type=int, help="Start index for training set.", default=0
    )
    parser.add_argument(
        "--end_training", type=int, help="End index for training set.", default=-1
    )
    parser.add_argument(
        "--delete",
        action="store_false",
        help="Delete the original files after processing.",
    )
    ...
    def log_file_list(name, file_list):
        log_message = f"{name} files:\n" + "\n".join(
            f"{i}: {s}" for i, s in enumerate(file_list)
        )
